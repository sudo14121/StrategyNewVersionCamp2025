...
class Drawer(BaseProcessor):
    ...
    def process(self) -> None:
        ...
        if field.robot_with_ball is not None:
            field.field_image.draw_dot(
                field.robot_with_ball.get_pos() + aux.rotate(aux.RIGHT, field.robot_with_ball.get_angle()) * 80,
                size_in_mms=const.ROBOT_R / 2,
            )
        ...
        for image_box in self.images:
            if image_box[1].is_pressed:
                for rect in image_box[0].rectangles:
